defmodule ChasingSun.Accounts.Scope do
  ...
  def permissions(:admin), do: [:view_dashboard, :manage_greenhouses, :manage_harvest, :manage_crop_rules, :delete_greenhouses]
  def permissions(:operator), do: [:view_dashboard, :manage_greenhouses, :manage_harvest]
  def permissions(:viewer), do: [:view_dashboard]

  def label(%User{role: role}) when is_atom(role), do: role |> Atom.to_string() |> String.capitalize()
